...
public:
	Scale(float domain_min, float domain_max, float range_min, float range_max) : domain_min(domain_min), domain_max(domain_max), range_min(range_min), range_max(range_max) {}
...
__global__
void kernel_scale(float* d_point_out, float* d_point_in, int samples, int d, int x_dim, int y_dim, int sample_interval, int entries_per_thread,
	...
	for (int j_ = (threadIdx.x + blockDim.x * blockIdx.x) * entries_per_thread; j_ < samples; j_ += blockDim.x * gridDim.x * entries_per_thread) {
		for (int j = j_;j < min(samples, j_ + entries_per_thread);j++) {
			int p = j * sample_interval;
			d_point_out[j * 2 + 0] = x_offset + x_range_min + (x_range_max - x_range_min) * (d_point_in[p * d + x_dim] - x_domain_min) / (x_domain_max - x_domain_min);
			d_point_out[j * 2 + 1] = y_offset + y_range_min + (y_range_max - y_range_min) * (d_point_in[p * d + y_dim] - y_domain_min) / (y_domain_max - y_domain_min);
		}
	}
}

__global__
void kernel_color(float* __restrict__  d_colors, const float* __restrict__  d_points, const int* __restrict__ d_C, const int* __restrict__ d_C_sel, const bool* __restrict__  d_D, const int samples, const int d, const int k, const int x_dim, const int y_dim, const int sample_interval, const int entries_per_thread,
	const int selected_c, const float range_x_min, const float range_x_max, const float range_y_min, const float range_y_max, const int range_x, const int range_y) {
	...
	for (int j_ = (threadIdx.x + blockDim.x * blockIdx.x)* entries_per_thread; j_ < samples; j_ += blockDim.x * gridDim.x * entries_per_thread) {
		for (int j = j_;j < min(samples, j_ + entries_per_thread);j++) {
			...
			const bool hide = (selected_c != -2 && c != selected_c) || (range_x_min != range_x_max && range_y_min != range_y_max && !(range_x_min <= x && x <= range_x_max && range_y_min <= y && y <= range_y_max));
...
__global__
void kernel_scale_color(float* __restrict__ d_points_out, float* __restrict__  d_colors, const float* __restrict__  d_points, const int* __restrict__ d_C, const int* __restrict__ d_C_sel, const bool* __restrict__  d_D, const int samples, const int d, const int k, const int x_dim, const int y_dim, const int sample_interval, const int entries_per_thread,
	const int selected_c, const float range_x_min, const float range_x_max, const float range_y_min, const float range_y_max, const int range_x, const int range_y,
	GLint x_offset, GLint y_offset,
	float x_domain_min, float x_domain_max, float x_range_min, float x_range_max,
	float y_domain_min, float y_domain_max, float y_range_min, float y_range_max) {
	...
	for (int j_ = (threadIdx.x + blockDim.x * blockIdx.x) * entries_per_thread; j_ < samples; j_ += blockDim.x * gridDim.x * entries_per_thread) {
		for (int j = j_;j < min(samples, j_ + entries_per_thread);j++) {
			...
			d_points_out[j * 2 + 0] = x_offset + x_range_min + (x_range_max - x_range_min) * (d_points[p * d + x_dim] - x_domain_min) / (x_domain_max - x_domain_min);
			d_points_out[j * 2 + 1] = y_offset + y_range_min + (y_range_max - y_range_min) * (d_points[p * d + y_dim] - y_domain_min) / (y_domain_max - y_domain_min);
...
private:
	...
	nvstd::function<int(int*, int d, int p, int x_dim, int y_dim)> transform = nullptr; //[&] __device__(T1* d_Data, int d, int p)->T2 { return NULL; };
...
public:
	...
	void paint() {
		...
		*/
		...
		for (int i = 0;i < count;i++) {
			glColor3f(context.colors[i * 3 + 0] / 255., context.colors[i * 3 + 1] / 255., context.colors[i * 3 + 2] / 255.); //Set the colour to red 
			float height = y_scale->get_range_min() + (y_scale->get_range_max() - y_scale->get_range_min()) * (h_out[i] - y_scale->get_domain_min()) / (y_scale->get_domain_max() - y_scale->get_domain_min());
...
public:
	Points(Scale* x_scale, Scale* y_scale, float* d_points, float* h_points, int n, int d) :x_scale(x_scale), y_scale(y_scale), d_points(d_points), h_points(h_points), n(n), d(d) {}
	...
	void paint() {
		...
		if (context.use_GPU) {
			...
			kernel_scale << <number_of_blocks, min(samples, block_size) >> > (cuVertex, d_points, samples, d, x_dim, y_dim, sample_interval, entries_per_thread,
				x_offset, y_offset,
				x_scale->get_domain_min(), x_scale->get_domain_max(), x_scale->get_range_min(), x_scale->get_range_max(),
				y_scale->get_domain_min(), y_scale->get_domain_max(), y_scale->get_range_min(), y_scale->get_range_max());
			...
			kernel_color << <number_of_blocks, min(samples, block_size) >> > (d_colors, d_points, context.get_result(get_k(), get_l()).d_C, context.get_result(context.k, context.l).d_C, context.get_result(get_k(), get_l()).d_D, samples, d, get_k(), x_dim, y_dim, sample_interval, entries_per_thread,
			...
			for (int j = 0; j < samples; j++) {
				...
				// scale points
				float x_scaled = x_offset + x_scale->get_range_min() + (x_scale->get_range_max() - x_scale->get_range_min()) * (h_points[p * d + x_dim] - x_scale->get_domain_min()) / (x_scale->get_domain_max() - x_scale->get_domain_min());
				float y_scaled = y_offset + y_scale->get_range_min() + (y_scale->get_range_max() - y_scale->get_range_min()) * (h_points[p * d + y_dim] - y_scale->get_domain_min()) / (y_scale->get_domain_max() - y_scale->get_domain_min());
				...
				if (context.range_x_min != context.range_x_max && context.range_y_min != context.range_y_max) {
					...
					if (context.range_x_min <= x && x <= context.range_x_max && context.range_y_min <= y && y <= context.range_y_max) {
...
void kernel_heatmap_color(float* d_colors, float* d_points, int n, int d, int x_dim, int y_dim,
	...
	float range_x_min, float range_x_max, float range_y_min, float range_y_max, int range_x, int range_y,
	float x_domain_min, float x_domain_max, float x_range_min, float x_range_max,
	float y_domain_min, float y_domain_max, float y_range_min, float y_range_max) {
	...
	for (int j_ = (threadIdx.x + blockDim.x * blockIdx.x) * entries_per_thread; j_ < n; j_ += blockDim.x * gridDim.x * entries_per_thread) {
		for (int j = j_; j < min(n, j_ + entries_per_thread);j++) {

			int c = d_C_sel[j];


			int x = (x_range_min + (x_range_max - x_range_min) * (d_points[j * d + x_dim] - x_domain_min) / (x_domain_max - x_domain_min)) / tile_x_width;
			int y = (y_range_min + (y_range_max - y_range_min) * (d_points[j * d + y_dim] - y_domain_min) / (y_domain_max - y_domain_min)) / tile_y_width;
			...
			if (selected_c == -2 || c == selected_c) {
				if (range_x_min != range_x_max && range_y_min != range_y_max) {
					...
					if (range_x_min <= x && x <= range_x_max && range_y_min <= y && y <= range_y_max) {
						for (int i = 0; i < 4; i++) {
							atomicAdd(&d_colors[idx * 4 * color_components + i * color_components + 3], ((float)width) / n);
						}
					}
				}
				else {
...
public:
	...
	Heatmap(Scale* x_scale, Scale* y_scale, float* d_points, float* h_points, int n, int d) :x_scale(x_scale), y_scale(y_scale), d_points(d_points), h_points(h_points), n(n), d(d) {}
	...
	void paint() {
		...
		if (context.use_GPU) {
			...
			kernel_heatmap_color_3 << <number_of_blocks, min(n, block_size), 32*32*sizeof(float) >> > (d_colors, d_points, n, d, x_dim, y_dim, context.get_result(context.k, context.l).d_C, context.selected_c,
			...
			for (int p = 0;p < n;p++) {
				...
				int x = (x_scale->get_range_min() + (x_scale->get_range_max() - x_scale->get_range_min()) * (h_points[p * d + x_dim] - x_scale->get_domain_min()) / (x_scale->get_domain_max() - x_scale->get_domain_min())) / tile_x_width;
				int y = (y_scale->get_range_min() + (y_scale->get_range_max() - y_scale->get_range_min()) * (h_points[p * d + y_dim] - y_scale->get_domain_min()) / (y_scale->get_domain_max() - y_scale->get_domain_min())) / tile_y_width;
			...
			for (int x = 0; x < 32; x++) {
				for (int y = 0; y < 32;y++) {
					...
					glVertex2i(x_offset + x * tile_x_width, y_offset + y * tile_y_width);            //Draw the four corners of the rectangle
...
void kernel_small_points_color(float* d_colors, float* d_points, int n, int d, int x_dim, int y_dim,
	...
	for (int j = threadIdx.x + blockDim.x * blockIdx.x; j < n; j += blockDim.x * gridDim.x) {
		...
		int px_x = (x_range_min + (x_range_max - x_range_min) * (d_points[j * d + x_dim] - x_domain_min) / (x_domain_max - x_domain_min));
		int px_y = (y_range_min + (y_range_max - y_range_min) * (d_points[j * d + y_dim] - y_domain_min) / (y_domain_max - y_domain_min));
		...
		bool hide = (selected_c != -2 && c != selected_c) || (range_x_min != range_x_max && range_y_min != range_y_max && !(range_x_min <= x && x <= range_x_max && range_y_min <= y && y <= range_y_max));
...
public:
	...
	Small_points(Scale* x_scale, Scale* y_scale, float* d_points, float* h_points, int n, int d) :x_scale(x_scale), y_scale(y_scale), d_points(d_points), h_points(h_points), n(n), d(d) {}
	...
	void paint() {
		...
		if (context.use_GPU) {
			...
			int number_of_blocks = n / 1024;
			if (n % number_of_blocks) number_of_blocks++;
			kernel_small_points_color << <number_of_blocks, min(n, 1024) >> > (d_colors, d_points, n, d, x_dim, y_dim, context.get_result(get_k(), get_l()).d_C, context.get_result(context.k, context.l).d_C, context.get_result(context.k, context.l).d_D, context.selected_c,
				width, height,
				context.range_x_min, context.range_x_max, context.range_y_min, context.range_y_max, context.range_x, context.range_y,
				x_scale->get_domain_min(), x_scale->get_domain_max(), x_scale->get_range_min(), x_scale->get_range_max(),
				y_scale->get_domain_min(), y_scale->get_domain_max(), y_scale->get_range_min(), y_scale->get_range_max());
...
__global__
void kernel_density(float* d_density, float* d_points, int n, int d, int x_width, int y_width, int x_offset, int y_offset, int x_dim, int y_dim,
	float x_domain_min, float x_domain_max, float x_range_min, float x_range_max,
	float y_domain_min, float y_domain_max, float y_range_min, float y_range_max) {
	int px_x = blockIdx.x;
	int px_y = blockIdx.y;
	...
	for (int p = threadIdx.x; p < n; p += blockDim.x) {
		...
		float p_x = x_range_min + (x_range_max - x_range_min) * (d_points[p * d + x_dim] - x_domain_min) / (x_domain_max - x_domain_min);
		float p_y = y_range_min + (y_range_max - y_range_min) * (d_points[p * d + y_dim] - y_domain_min) / (y_domain_max - y_domain_min);
...
public:
	...
	Density(Scale* x_scale, Scale* y_scale, float* d_points, int n, int d) :x_scale(x_scale), y_scale(y_scale), d_points(d_points), n(n), d(d) {
	...
	void build(int new_x_offset, int new_y_offset) {
		...
		kernel_density << <grid, min(n, 1024) >> > (d_density, d_points, n, d, width, height, x_offset, y_offset, x_dim, y_dim,
			x_scale->get_domain_min(), x_scale->get_domain_max(), x_scale->get_range_min(), x_scale->get_range_max(),
			y_scale->get_domain_min(), y_scale->get_domain_max(), y_scale->get_range_min(), y_scale->get_range_max());
	}
...
public:
	...
	void convert_range() {
		context.range_x_min = context.range_x_start < context.range_x_end ? context.range_x_start : context.range_x_end;
		context.range_x_max = context.range_x_start < context.range_x_end ? context.range_x_end : context.range_x_start;
		context.range_y_min = context.range_y_start < context.range_y_end ? context.range_y_start : context.range_y_end;
		context.range_y_max = context.range_y_start < context.range_y_end ? context.range_y_end : context.range_y_start;
		...
		context.range_x_min = (context.range_x_min - x_scale->get_range_min()) / (x_scale->get_range_max() - x_scale->get_range_min()) * (x_scale->get_domain_max() - x_scale->get_domain_min()) + x_scale->get_domain_min();
		context.range_x_max = (context.range_x_max - x_scale->get_range_min()) / (x_scale->get_range_max() - x_scale->get_range_min()) * (x_scale->get_domain_max() - x_scale->get_domain_min()) + x_scale->get_domain_min();
		context.range_y_min = (context.range_y_min - y_scale->get_range_min()) / (y_scale->get_range_max() - y_scale->get_range_min()) * (y_scale->get_domain_max() - y_scale->get_domain_min()) + y_scale->get_domain_min();
		context.range_y_max = (context.range_y_max - y_scale->get_range_min()) / (y_scale->get_range_max() - y_scale->get_range_min()) * (y_scale->get_domain_max() - y_scale->get_domain_min()) + y_scale->get_domain_min();
	...
	void build(int new_x_offset, int new_y_offset) {
		...
		y_axis->left();
		plt_row1->add(new Cell(y_axis_width, y_scale->get_range_max() - y_scale->get_range_min(), (Element*)y_axis));

		plt_row1->add(new Cell(x_scale->get_range_max() - x_scale->get_range_min(), y_scale->get_range_max() - y_scale->get_range_min(), (Element*)content_group));

		plt_row2->add(new Cell(x_axis_height, y_axis_width));

		x_axis->set_tick_height(x_axis_height);
		plt_row2->add(new Cell(x_scale->get_range_max() - x_scale->get_range_min(), x_axis_height, (Element*)x_axis));
	...
	void paint() {
		...
		if (range_selection && context.range_x_start != context.range_x_end && context.range_y_start != context.range_y_end &&
			context.range_x == context.selected_i &&
			context.range_y == context.selected_j) {
...
public:
	...
	void paint() {
		...
		float hover_radius = track_width < track_width/2. + track_width / (500. / hover_counter ) ? track_width : track_width / 2. + track_width / (500. / hover_counter + .5);
...
Table* build_scatter_plot_matrix(int scatter_plot_matrix_size) {
	...
	for (int j = 0; j < context.d; j++) {
		...
		for (int i = 0; i < context.d; i++) {
			...
			if (i != j) {
				Scale* x_scale = new Scale(context.min_values[i], context.max_values[i], 0, plot_full_size - plot_full_margin);
				Scale* y_scale = new Scale(context.min_values[j], context.max_values[j], 0, plot_full_size - plot_full_margin);
				...
				if (i > j) {//(i > j || (!heatmap && !density)) {
					...
					Small_points* pts = new Small_points(x_scale, y_scale, context.d_points_in, context.h_points_in, context.n, context.d);
					pts->set_dims(i, j);
					...
					if (heatmap) {
						Heatmap* pts = new Heatmap(x_scale, y_scale, context.d_points_in, context.h_points_in, context.n, context.d);
						pts->set_dims(i, j);
						plt->add_content((Element*)pts);
...
Table* build_scatter_right_menu(int width, int height) {
	...
	for (int i = 0;i < 3;i++) {
		Scale* x_scale = new Scale(context.min_values[context.selected_i], context.max_values[context.selected_i], 0, width);
		Scale* y_scale = new Scale(context.min_values[context.selected_j], context.max_values[context.selected_j], 0, width);
		Plot* plt = new Plot(x_scale, y_scale);
...
Table* build_scatter_bottom_menu(int width, int height) {
	...
	for (int i = 0;i < 3;i++) {
		Scale* x_scale = new Scale(context.min_values[context.selected_i], context.max_values[context.selected_i], 0, height);
		Scale* y_scale = new Scale(context.min_values[context.selected_j], context.max_values[context.selected_j], 0, height);
		Plot* plt = new Plot(x_scale, y_scale);
...
void build_display() {
	...
	body_row->add(new Cell(body_height, body_height, (Element*)left_side));
	...
	cell22->set_content((Element*)info_table);
	...
	body_row->add(new Cell(right_side_width, body_height, (Element*)right_side));

	//right_side->add(new Row(new Cell(right_side_width, (body_height- right_side_width)/2)));

	Scale* x_scale = new Scale(context.min_values[context.selected_i], context.max_values[context.selected_i], 0, right_side_width - 30);
	Scale* y_scale = new Scale(context.min_values[context.selected_j], context.max_values[context.selected_j], 0, right_side_width - 30);
	Plot* plt = new Plot(x_scale, y_scale);
	...
	plt->set_x_axis_height(30);
	plt->set_y_axis_width(30);
	...
	right_side->add(new Row(new Cell(right_side_width, right_side_width, (Element*)plt)));

	//right_side->add(new Row(new Cell(right_side_width, (body_height - right_side_width) / 2)));

	Table* menu = new Table();
	menu->set_spacing(right_side_spacing);
	int menu_row_height = body_height - right_side_width - right_side_spacing - 2 * body_margin;

	right_side->add(new Row(new Cell(right_side_width, menu_row_height, (Element*)menu)));
	...
	Cell* menu_row_cell1 = new Cell(cell_width, menu_row_height, (Element*)re_button);
	menu_row_cell1->v_align_center();
	menu_row_cell1->h_align_center();
	menu_row->add(menu_row_cell1);

	Text* GPU_vis_text = new Text(cell_width - right_side_spacing, menu_row_height);
	...
	Cell* menu_row_cell2 = new Cell(cell_width - right_side_spacing, menu_row_height, (Element*)GPU_vis_text);
	...
	Cell* menu_row_cell3 = new Cell(cell_width/2 - 2 * right_side_spacing, menu_row_height, (Element*)s);
	menu_row->add(menu_row_cell3);
	menu_row_cell3->v_align_center();

	Text* SPF_text = new Text(cell_width - right_side_spacing, menu_row_height);
	SPF_text->set_text([]() {
	...
	Cell* menu_row_cell4 = new Cell(cell_width - right_side_spacing, menu_row_height, (Element*)SPF_text);
...
void run_proclus() {
	...
	context.result = GPU_PROCLUS_SAVE(context.d_points_in, context.n, context.d, context.k, context.l, a, b, min_deviation, termination_rounds, false);
}
...
int main(int argc, char** argv) {  // Initialize GLUT 
	...
	if (fields.size() > 0 && fields[0].size() > 0) {
		...
		cudaMemcpy(context.d_points_in, context.h_points_in, context.n * context.d * sizeof(float), cudaMemcpyHostToDevice);
	...
	glutInitWindowSize(context.window_width + 17, context.window_height + 17);         // Size of display area, in pixels.
	...
	glBindBuffer(GL_ARRAY_BUFFER, context.glBuffer);
	glBufferData(GL_ARRAY_BUFFER, context.n * (2 + 4) * sizeof(float), NULL, GL_DYNAMIC_DRAW); //making room for location and color
